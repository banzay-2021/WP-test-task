/* global ritzBgColors, ritzColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( ritzBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( ritzBgColors[ context ].color ) {
						backgroundColorValue = ritzBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( ritzBgColors[ context ].setting ).get();
					}
					ritzSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( ritzBgColors ).forEach( function( context ) {
			wp.customize( ritzBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					ritzSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		ritzSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				ritzSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function ritzSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = ritzColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function ritzSetRetineLogoVisibility( visible ) {
