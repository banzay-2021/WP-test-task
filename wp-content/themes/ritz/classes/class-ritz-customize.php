...
if ( ! class_exists( 'Ritz_Customize' ) ) {
	/**
	 * CUSTOMIZER SETTINGS
	 *
	 * @since Twenty Twenty 1.0
	 */
	class Ritz_Customize {
		...
		public static function register( $wp_customize ) {
			...
			$wp_customize->selective_refresh->add_partial(
				...
				array(
					...
					'render_callback'     => 'ritz_customize_partial_site_logo',
			...
			$wp_customize->add_control(
				...
				array(
					...
					'priority'    => 10,
					'label'       => __( 'Retina logo', 'ritz' ),
					'description' => __( 'Scales the logo to half its uploaded size, making it sharp on high-res screens.', 'ritz' ),
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					'header_footer_background_color',
					array(
						'label'   => __( 'Header &amp; Footer Background Color', 'ritz' ),
						'section' => 'colors',
					)
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					'type'    => 'radio',
					'section' => 'colors',
					'label'   => __( 'Primary Color', 'ritz' ),
					'choices' => array(
						'default' => _x( 'Default', 'color', 'ritz' ),
						'custom'  => _x( 'Custom', 'color', 'ritz' ),
					),
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					array(
						'section'         => 'colors',
						'settings'        => 'accent_hue',
						'description'     => __( 'Apply a custom color for links, buttons, featured images.', 'ritz' ),
						'mode'            => 'hue',
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'Show search in header', 'ritz' ),
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'On archive pages, posts show:', 'ritz' ),
					'choices'  => array(
						'full'    => __( 'Full text', 'ritz' ),
						'summary' => __( 'Summary', 'ritz' ),
					),
				)
			);
			...
			$wp_customize->add_section(
				...
				array(
					'title'       => __( 'Cover Template', 'ritz' ),
					'capability'  => 'edit_theme_options',
					'description' => __( 'Settings for the "Cover Template" page template. Add a featured image to use as background.', 'ritz' ),
					'priority'    => 42,
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'label'       => __( 'Fixed Background Image', 'ritz' ),
					'description' => __( 'Creates a parallax effect when the visitor scrolls.', 'ritz' ),
				)
			);
			...
			$wp_customize->add_setting(
				...
				array(
					'default'           => ritz_get_color_for_area( 'content', 'accent' ),
					'sanitize_callback' => 'sanitize_hex_color',
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					'cover_template_overlay_background_color',
					array(
						'label'       => __( 'Overlay Background Color', 'ritz' ),
						'description' => __( 'The color used for the overlay. Defaults to the accent color.', 'ritz' ),
						'section'     => 'cover_template_options',
					)
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					'cover_template_overlay_text_color',
					array(
						'label'       => __( 'Overlay Text Color', 'ritz' ),
						'description' => __( 'The color used for the text in the overlay.', 'ritz' ),
						'section'     => 'cover_template_options',
					)
				)
			);
			...
			$wp_customize->add_control(
				'cover_template_overlay_opacity',
				array(
					'label'       => __( 'Overlay Opacity', 'ritz' ),
					'description' => __( 'Make sure that the contrast is high enough so that the text is readable.', 'ritz' ),
					'section'     => 'cover_template_options',
					'type'        => 'range',
					'input_attrs' => ritz_customize_opacity_range(),
				)
			);
	...
	add_action( 'customize_register', array( 'Ritz_Customize', 'register' ) );
 ...
 * */
if ( ! function_exists( 'ritz_customize_partial_blogname' ) ) {
	/**
...
if ( ! function_exists( 'ritz_customize_partial_blogdescription' ) ) {
	/**
...
if ( ! function_exists( 'ritz_customize_partial_site_logo' ) ) {
	/**
	 ...
	 * @since Twenty Twenty 1.0
	 */
	function ritz_customize_partial_site_logo() {
		ritz_site_logo();
	}
}
 ...
 */
function ritz_customize_opacity_range() {
	/**
