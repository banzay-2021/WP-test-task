 ...
 */
function ritz_site_logo( $args = array(), $echo = true ) {
	...
	/**
	 * Filters the arguments for `ritz_site_logo()`.
	 *
	 * @since Twenty Twenty 1.0
	 *
	 ...
	 */
	$args = apply_filters( 'ritz_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'ritz_site_logo', $html, $args, $classname, $contents );
 ...
 */
function ritz_site_description( $echo = true ) {
	 ...
	 */
	$html = apply_filters( 'ritz_site_description', $html, $description, $wrapper );
 ...
 * @return bool
 */
function ritz_is_comment_by_post_author( $comment = null ) {
 ...
 */
function ritz_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'ritz_filter_comment_reply_link' );
 ...
 */
function ritz_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo ritz_get_post_meta( $post_id, $location ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in ritz_get_post_meta().
 ...
 */
function ritz_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$text = sprintf(
		wp_kses(
			...
			__( 'Edit <span class="screen-reader-text">%s</span>', 'ritz' ),
			array(
				'span' => array(
					'class' => array(),
				),
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . ritz_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter( 'edit_post_link', 'ritz_edit_post_link', 10, 3 );
 ...
 */
function ritz_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'ritz_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		$post_meta = apply_filters(
			'ritz_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		 ...
		 */
		$post_meta = apply_filters(
			'ritz_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ( $post_meta && ! in_array( 'empty', $post_meta, true ) ) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr( $post_meta_wrapper_classes ); ?>">
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				 ...
				 */
				do_action( 'ritz_start_of_post_meta_list', $post_id, $post_meta, $location );
				...
				if ( post_type_supports( get_post_type( $post_id ), 'author' ) && in_array( 'author', $post_meta, true ) ) {
					...
					<li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post author', 'ritz' ); ?></span>
							<?php ritz_the_theme_svg( 'user' ); ?>
						</span>
				...
				if ( in_array( 'post-date', $post_meta, true ) ) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post date', 'ritz' ); ?></span>
							<?php ritz_the_theme_svg( 'calendar' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'categories', $post_meta, true ) && has_category() ) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Categories', 'ritz' ); ?></span>
							<?php ritz_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
							<?php _ex( 'In', 'A string that is output before one or more categories', 'ritz' ); ?> <?php the_category( ', ' ); ?>
						</span>
					</li>
					<?php
				...
				if ( in_array( 'tags', $post_meta, true ) && has_tag() ) {
					...
					<li class="post-tags meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Tags', 'ritz' ); ?></span>
							<?php ritz_the_theme_svg( 'tag' ); ?>
						</span>
						<span class="meta-text">
							<?php the_tags( '', ', ', '' ); ?>
						</span>
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php ritz_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php ritz_the_theme_svg( 'bookmark' ); ?>
						</span>
						<span class="meta-text">
							<?php _e( 'Sticky post', 'ritz' ); ?>
						</span>
					</li>
					<?php
				 ...
				 *                          Accepts 'single-top' or 'single-bottom'.
				 */
				do_action( 'ritz_end_of_post_meta_list', $post_id, $post_meta, $location );
 ...
 */
function ritz_filter_wp_list_pages_item_classes( $css_class, $page, $depth, $args, $current_page ) {
...
add_filter( 'page_css_class', 'ritz_filter_wp_list_pages_item_classes', 10, 5 );
 ...
 */
function ritz_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$toggle_duration      = ritz_toggle_duration();

			// Add the sub menu toggle.
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'ritz' ) . '</span>' . ritz_get_theme_svg( 'chevron-down' ) . '</button>';
...
add_filter( 'nav_menu_item_args', 'ritz_add_sub_toggles_to_main_menu', 10, 3 );
 ...
 */
function ritz_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = Ritz_SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
			$svg = ritz_get_theme_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'ritz_nav_menu_social_icons', 10, 4 );
...
add_action( 'wp_head', 'ritz_no_js_class' );
 ...
 */
function ritz_get_the_archive_title( $title ) {
	 ...
	 */
	$regex = apply_filters(
		'ritz_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'ritz_get_the_archive_title' );
...
function ritz_toggle_duration() {
	 ...
	 */
	$duration = apply_filters( 'ritz_toggle_duration', 250 );
 ...
 */
function ritz_unique_id( $prefix = '' ) {
	static $id_counter = 0;
